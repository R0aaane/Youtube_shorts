...
SKILL_EFFECTS_DIR = PROJECT_ROOT / "assets" / "skill_effects"
...
@dataclass
class SushiProjectile:
    kind: str
    position: pygame.Vector2
    velocity: pygame.Vector2
    radius: int = 17
    frames_left: int = 120
    owner_skin: str = "sushi"
...
SKILL_EFFECT_CACHE: dict[str, pygame.Surface] = {}
...
FOOD_BALL_STYLES = {
    ...
    "ice_cream": {"label": "ICE", "color": (178, 232, 255)},
...
def draw_scaled_food_skin(surface: pygame.Surface, skin: str, position: tuple[int, int], radius: int) -> None:
    ...
    if skin == "ice_cream":
        pygame.draw.polygon(surface, (206, 127, 48), [point(-42, 22), point(42, 22), point(0, 98)])
        pygame.draw.polygon(surface, (139, 79, 34), [point(-42, 22), point(42, 22), point(0, 98)], width=max(4, round(7 * scale)))
        pygame.draw.circle(surface, (248, 236, 211), point(0, -8), round(47 * scale))
        pygame.draw.circle(surface, (247, 149, 174), point(0, -58), round(45 * scale))
        pygame.draw.arc(surface, (130, 224, 255), rect(-70, -78, 140, 128), 0.2, 2.95, max(5, round(9 * scale)))
        return
...
def load_food_sprite(skin: str, radius: int, missing_stage: int = 0, cheese_stage: int = 0, ice_stage: int = 0) -> pygame.Surface | None:
    ...
    if skin == "burger" and missing_stage > 0:
        ...
        sprite_paths = {
            ...
            "sushi": FOOD_SPRITES_DIR / "sushi_alpha.png",
            "ice_cream": FOOD_SPRITES_DIR / "ice_cream_alpha.png",
        }
...
def trim_alpha_surface(surface: pygame.Surface) -> pygame.Surface:
...
def load_skill_effect_sprite(kind: str, size: int) -> pygame.Surface | None:
    cache_key = f"skill:{kind}:{size}"
    if cache_key in SKILL_EFFECT_CACHE:
        return SKILL_EFFECT_CACHE[cache_key]

    sheet_path = SKILL_EFFECTS_DIR / "food_skill_sheet_alpha.png"
    if not sheet_path.exists():
        return None
    sheet = load_alpha_surface(sheet_path)
    half_width = sheet.get_width() // 2
    half_height = sheet.get_height() // 2
    quadrants = {
        "cheese": pygame.Rect(0, 0, half_width, half_height),
        "wasabi": pygame.Rect(half_width, 0, half_width, half_height),
        "soy": pygame.Rect(0, half_height, half_width, half_height),
        "heal": pygame.Rect(half_width, half_height, half_width, half_height),
    }
    source_rect = quadrants.get(kind)
    if source_rect is None:
        return None
    cropped = pygame.Surface(source_rect.size, pygame.SRCALPHA)
    cropped.blit(sheet, (0, 0), source_rect)
    trimmed = trim_alpha_surface(cropped)
    if trimmed.get_width() <= 0 or trimmed.get_height() <= 0:
        return None
    scale = size / max(trimmed.get_width(), trimmed.get_height())
    scaled_size = (max(1, round(trimmed.get_width() * scale)), max(1, round(trimmed.get_height() * scale)))
    scaled = pygame.transform.smoothscale(trimmed, scaled_size)
    SKILL_EFFECT_CACHE[cache_key] = scaled
    return scaled


def blit_center(surface: pygame.Surface, image: pygame.Surface, center: tuple[int, int], alpha: int = 255) -> None:
    draw_image = image.copy()
    draw_image.set_alpha(max(0, min(255, alpha)))
    surface.blit(draw_image, draw_image.get_rect(center=center))
...
def draw_food_sprite(surface: pygame.Surface, ball: DuelBall, position: tuple[int, int]) -> bool:
    ...
    outline = rotated.copy()
    outline.fill((255, 250, 232, 205), special_flags=pygame.BLEND_RGBA_MULT)
...
def draw_duel_ball(surface: pygame.Surface, ball: DuelBall, font: pygame.font.Font) -> None:
    ...
    if ball.charge_frames > 0 and len(ball.trail) > 1:
        for index, trail_position in enumerate(ball.trail[-8:]):
            ...
            if ball.skin == "sushi":
                base_trail = pygame.Color(196, 245, 255)
                trail_color = base_trail.lerp(pygame.Color(255, 255, 255), ratio * 0.45)
                pygame.draw.circle(surface, trail_color, (round(trail_position.x), round(trail_position.y)), trail_radius, width=max(5, round(12 * ratio)))
                if ball.velocity.length_squared() > 0:
                    dash_dir = -ball.velocity.normalize()
                    for side in [-1, 1]:
                        start = trail_position + dash_dir.rotate(16 * side) * (ball.radius * 0.45)
                        end = trail_position + dash_dir.rotate(8 * side) * (ball.radius * (1.35 + ratio))
                        pygame.draw.line(surface, (220, 255, 255), start, end, max(3, round(7 * ratio)))
            else:
                base_trail = pygame.Color(255, 76, 70)
                trail_color = base_trail.lerp(pygame.Color(255, 230, 210), ratio * 0.35)
                pygame.draw.circle(surface, trail_color, (round(trail_position.x), round(trail_position.y)), trail_radius, width=max(4, round(10 * ratio)))
    ...
    if ball.wasabi_frames > 0:
        status = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
        pulse = 1 + math.sin(ball.wasabi_frames * 0.28) * 0.06
        pygame.draw.circle(status, (46, 204, 66, 124), position, round(ball.radius * 1.26 * pulse), width=8)
        pygame.draw.circle(status, (112, 255, 108, 62), position, round(ball.radius * 0.92 * pulse), width=4)
        for index in range(18):
            angle = ball.wasabi_frames * 5 + index * 20
            dot = pygame.Vector2(position) + pygame.Vector2(1, 0).rotate(angle) * (ball.radius * (0.76 + (index % 4) * 0.11))
            pygame.draw.circle(status, (100, 255, 94, 190), (round(dot.x), round(dot.y)), 5 + index % 4)
            if index % 3 == 0:
                end = dot + pygame.Vector2(1, 0).rotate(angle + 42) * 18
                pygame.draw.line(status, (184, 255, 128, 132), dot, end, 3)
        pygame.draw.circle(status, (52, 190, 58, 238), (position[0] - ball.radius - 14, position[1] - ball.radius - 10), 15)
    ...
    if ball.charge_frames > 0:
        ...
        label = "ROLL DASH" if ball.skin == "sushi" else "COLD DASH" if ball.skin == "ice_cream" else "CHARGE"
        draw_text_with_shadow(surface, label_font, label, (position[0], position[1] - ball.radius - 76), duel_attack_color(ball.skin))
        ...
        draw_text_with_shadow(surface, label_font, "BURN", (position[0], position[1] - ball.radius - 76), (255, 172, 58))
    elif ball.reload_frames > 0:
        pulse = 1 + math.sin(ball.reload_frames * 0.22) * 0.1
        label_font = make_font(round(42 * pulse), bold=True, italic=True)
...
def draw_skill_label(
    ...
    scale = 1.28 - progress * 0.18 + math.sin(progress * math.pi) * 0.16
    ...
    for dx, dy in [(-5, 0), (5, 0), (0, -5), (0, 5), (-4, -4), (4, -4), (-4, 4), (4, 4)]:
        surface.blit(outline_surface, rect.move(dx, dy))
...
def draw_duel_effects(surface: pygame.Surface, effects: list[DuelEffect]) -> None:
    for effect in effects:
        progress = max(0.0, min(1.0, 1 - effect.frames_left / max(1, 36)))
        center = (round(effect.position.x), round(effect.position.y))
        if effect.kind == "impact":
            strong = bool(effect.label) or effect.frames_left >= 26
            critical = effect.label == "CRITICAL!"
            alpha = max(0, min(230, round((230 if strong else 210) * (1 - progress))))
            flash = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
            if strong:
                flash.fill((255, 242, 210, round((42 if effect.label else 24) * (1 - progress))))
            radius = round(90 + progress * (380 if critical else 310 if effect.label else 235))
            pygame.draw.circle(flash, (*effect.color, alpha), center, radius, width=20 if critical else 16 if strong else 13)
            pygame.draw.circle(flash, (255, 244, 214, round(alpha * 0.84)), center, max(20, round(radius * 0.58)), width=8)
            ...
            draw_duel_particles(surface, center, effect.color, progress, 14 if strong else 8, radius * 0.58)
            if effect.label:
                label_center = (WIDTH // 2, round(HEIGHT * 0.38)) if critical else (center[0], center[1] - radius - 48)
                draw_skill_label(surface, effect.label, label_center, effect.color, progress, 68 if critical else 48)
            ...
            flash = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
            pygame.draw.circle(flash, (238, 255, 255, round(160 * (1 - progress))), center, radius + 14, width=4)
            pygame.draw.circle(flash, (*effect.color, round(210 * (1 - progress))), center, radius, width=10)
            for angle in range(0, 360, 30):
                start = pygame.Vector2(center) + pygame.Vector2(1, 0).rotate(angle) * (radius * 0.25)
                end = pygame.Vector2(center) + pygame.Vector2(1, 0).rotate(angle) * (radius * 0.92)
                pygame.draw.line(flash, (230, 255, 255, round(180 * (1 - progress))), start, end, 5)
            surface.blit(flash, (0, 0))
            draw_duel_particles(surface, center, effect.color, progress, 10, radius * 0.58)
            if effect.label and should_draw_skill_label(effect.label):
                draw_skill_label(surface, effect.label, (center[0], center[1] - radius - 56), effect.color, progress)
        elif effect.kind == "cheese":
            cheese = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
            radius = round(34 + progress * 65)
            sprite = load_skill_effect_sprite("cheese", round(radius * 2.35))
            ...
            if effect.label and should_draw_skill_label(effect.label):
                draw_skill_label(surface, effect.label, (center[0], center[1] - radius - 56), effect.color, progress, 38)
        elif effect.kind == "wasabi":
            wasabi = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
            radius = round(30 + progress * 76)
            sprite = load_skill_effect_sprite("wasabi", round(radius * 2.25))
            if sprite is not None:
                blit_center(wasabi, sprite, center, round(210 * (1 - progress)))
            for angle in range(0, 360, 40):
                end = pygame.Vector2(center) + pygame.Vector2(1, 0).rotate(angle + progress * 24) * radius * (0.55 + (angle % 4) * 0.09)
                pygame.draw.line(wasabi, (44, 208, 58, round(178 * (1 - progress))), center, end, max(5, round(12 * (1 - progress))))
                pygame.draw.circle(wasabi, (112, 255, 92, round(205 * (1 - progress))), (round(end.x), round(end.y)), max(5, round(9 * (1 - progress))))
            pygame.draw.circle(wasabi, (56, 212, 64, round(142 * (1 - progress))), center, radius, width=10)
            surface.blit(wasabi, (0, 0))
            draw_duel_particles(surface, center, (92, 244, 78), progress, 16, radius * 0.78)
            if effect.label and should_draw_skill_label(effect.label):
                draw_skill_label(surface, effect.label, (center[0], center[1] - radius - 56), effect.color, progress, 38)
        ...
        elif effect.kind == "soy":
            splash = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
            radius = round(30 + progress * 72)
            sprite = load_skill_effect_sprite("soy", round(radius * 2.35))
            if sprite is not None:
                blit_center(splash, sprite, center, round(190 * (1 - progress)))
            pygame.draw.ellipse(
                splash,
                (118, 70, 38, round(140 * (1 - progress))),
                pygame.Rect(center[0] - round(radius * 1.15), center[1] + radius // 3, round(radius * 2.3), max(18, round(radius * 0.62))),
                width=8,
            )
            pygame.draw.circle(splash, (124, 74, 40, round(142 * (1 - progress))), center, radius)
            for angle in range(0, 360, 24):
                end = pygame.Vector2(center) + pygame.Vector2(1, 0).rotate(angle) * radius * (0.78 + (angle % 5) * 0.08)
                pygame.draw.line(splash, (134, 80, 44, round(175 * (1 - progress))), center, end, max(5, round(12 * (1 - progress))))
                pygame.draw.circle(splash, (188, 128, 74, round(170 * (1 - progress))), (round(end.x), round(end.y)), max(5, round(10 * (1 - progress))))
            ...
            heal = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
            radius = round(34 + progress * 74)
            sprite = load_skill_effect_sprite("heal", round(radius * 2.4))
            if sprite is not None:
                blit_center(heal, sprite, center, round(185 * (1 - progress)))
            pygame.draw.circle(heal, (86, 232, 230, round(230 * (1 - progress))), center, radius, width=9)
            pygame.draw.circle(heal, (222, 255, 250, round(198 * (1 - progress))), center, max(14, radius // 2), width=6)
            pygame.draw.circle(heal, (112, 255, 210, round(120 * (1 - progress))), center, round(radius * 1.32), width=5)
            for index in range(18):
                angle = index * 20 + progress * 86
                bubble = pygame.Vector2(center) + pygame.Vector2(1, 0).rotate(angle) * radius * (0.45 + (index % 4) * 0.12)
                pygame.draw.circle(heal, (202, 255, 244, round(205 * (1 - progress))), (round(bubble.x), round(bubble.y)), max(4, round(9 * (1 - progress))))
                if index % 3 == 0:
                    draw_text_with_shadow(heal, make_font(max(18, round(24 * (1 - progress))), bold=True), "+", (round(bubble.x), round(bubble.y)), (222, 255, 250))
            surface.blit(heal, (0, 0))
            if effect.label and should_draw_skill_label(effect.label):
                draw_skill_label(surface, effect.label, (center[0], center[1] - radius - 78), effect.color, progress, 46)
...
def draw_duel_endgame_alerts(surface: pygame.Surface, left: DuelBall, right: DuelBall, frame_index: int) -> None:
    ...
    for index, ball in enumerate([left, right]):
        if ball.hp <= 0:
            continue
        ratio = ball.hp / max(1, ball.max_hp)
        if ratio > 0.25:
            continue
        label = "ONE HIT LEFT!" if ratio <= 0.10 else "DANGER!"
        color = (255, 68, 64) if ratio <= 0.10 else (255, 188, 52)
        pulse = 1 + math.sin(frame_index * 0.22) * 0.08
        font = make_font(round((58 if ratio <= 0.10 else 48) * pulse), bold=True, italic=True)
        position = (round(WIDTH * (0.25 if index == 0 else 0.75)), round(HEIGHT * 0.142))
...
def draw_cheese_projectiles(surface: pygame.Surface, projectiles: list[CheeseProjectile]) -> None:
    for projectile in projectiles:
        ...
        for index, angle in enumerate([0, 55, 118, 188, 252, 315]):
            scale = 0.78 + ((index * 31 + projectile.frames_left) % 5) * 0.06
            offset = pygame.Vector2(1, 0).rotate(angle) * projectile.radius * scale
            points.append((round(center[0] + offset.x), round(center[1] + offset.y)))
        pygame.draw.polygon(layer, (255, 180, 24, 242), points)
        pygame.draw.circle(layer, (255, 225, 64, 248), center, round(projectile.radius * 0.84))
        pygame.draw.circle(layer, (255, 250, 186, 220), (center[0] - 5, center[1] - 5), max(5, projectile.radius // 3))
        pygame.draw.polygon(layer, (168, 105, 18, 170), points, width=3)
        sprite = load_skill_effect_sprite("cheese", projectile.radius * 3)
        if sprite is not None:
            blit_center(layer, sprite, center, 210)
        surface.blit(layer, (0, 0))


def draw_sushi_projectiles(surface: pygame.Surface, projectiles: list[SushiProjectile]) -> None:
    for projectile in projectiles:
        center = (round(projectile.position.x), round(projectile.position.y))
        layer = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
        direction = projectile.velocity.normalize() if projectile.velocity.length_squared() > 0 else pygame.Vector2(1, 0)
        if projectile.kind == "freeze":
            ...
            pygame.draw.circle(layer, (238, 255, 255, 230), center, round(projectile.radius * 0.58))
            pygame.draw.polygon(layer, (238, 255, 255, 168), points, width=3)
        elif projectile.kind == "soy":
            for index in range(10):
                tail = pygame.Vector2(center) - direction.rotate((index - 4.5) * 8) * projectile.radius * (0.82 + index * 0.3)
                pygame.draw.circle(layer, (124, 72, 38, max(42, 158 - index * 12)), (round(tail.x), round(tail.y)), max(6, round(projectile.radius * 0.42)))
            pygame.draw.ellipse(layer, (96, 56, 30, 228), pygame.Rect(center[0] - round(projectile.radius * 1.18), center[1] - round(projectile.radius * 0.58), round(projectile.radius * 2.36), round(projectile.radius * 1.16)))
            pygame.draw.circle(layer, (148, 88, 46, 226), center, round(projectile.radius * 1.08))
            pygame.draw.circle(layer, (204, 140, 78, 194), (center[0] - 5, center[1] - 5), max(5, round(projectile.radius * 0.42)))
            sprite = load_skill_effect_sprite("soy", projectile.radius * 3)
            if sprite is not None:
                blit_center(layer, sprite, center, 170)
        else:
            for index in range(9):
                tail = pygame.Vector2(center) - direction.rotate((index - 3) * 6) * projectile.radius * (0.9 + index * 0.24)
                pygame.draw.circle(layer, (64, 226, 72, max(48, 172 - index * 12)), (round(tail.x), round(tail.y)), max(4, projectile.radius // 4))
            points = []
            for index, angle in enumerate([0, 42, 108, 184, 246, 315]):
                scale = 0.72 + ((index * 29 + projectile.frames_left) % 6) * 0.07
                offset = pygame.Vector2(1, 0).rotate(angle) * projectile.radius * scale
                points.append((round(center[0] + offset.x), round(center[1] + offset.y)))
            pygame.draw.polygon(layer, (42, 178, 54, 242), points)
            pygame.draw.circle(layer, (98, 250, 86, 238), center, round(projectile.radius * 0.76))
            pygame.draw.circle(layer, (214, 255, 186, 215), (center[0] - 4, center[1] - 4), max(4, projectile.radius // 3))
            sprite = load_skill_effect_sprite("wasabi", projectile.radius * 3)
            if sprite is not None:
                blit_center(layer, sprite, center, 190)
        outline_color = (232, 255, 255, 170) if projectile.owner_skin == "ice_cream" else (238, 255, 232, 150)
        pygame.draw.circle(layer, outline_color, center, projectile.radius + 3, width=2)
        surface.blit(layer, (0, 0))
...
def draw_cheese_patches(surface: pygame.Surface, patches: list[CheesePatch], left: DuelBall, right: DuelBall) -> None:
    for patch in patches:
        ...
        for angle, length_scale in [(75, 0.72), (130, 0.56), (250, 0.64)]:
            ...
            end = pygame.Vector2(center) + pygame.Vector2(1, 0).rotate(angle + target.visual_angle) * (base_radius * (1.25 + length_scale))
            pygame.draw.line(cheese, (255, 202, 34, 232), start, end, max(9, round(base_radius * 0.24)))
...
def apply_duel_skill(
    ...
    elif attacker.skin == "sushi" and attacker.charge_frames > 0:
        damage += 40
        effects.append(DuelEffect("charge", defender.position.copy(), 30, duel_attack_color("sushi"), "ROLL DASH"))
        audio_events.append(AudioEvent(frame_index, "charge_hit"))
...
def update_duel_ball(
    ...
    sushi_projectiles: list[SushiProjectile],
    effects: list[DuelEffect],
) -> None:
    ...
    if ball.skin == "burger" and ball.skill_cooldown == 0:
        ...
        if direction.length_squared() > 0:
            if ball.reload_frames > 0:
                ...
                ball.skill_cooldown = 102
                audio_events.append(AudioEvent(frame_index, "charge_start"))
    elif ball.skin == "sushi" and ball.skill_cooldown == 0:
        direction = target.position - ball.position
        if direction.length_squared() == 0:
            direction = pygame.Vector2(1, 0)
        direction = direction.normalize()
        skill = ["wasabi", "soy", "roll", "heal"][ball.sushi_skill_index % 4]
        ball.sushi_skill_index += 1
        if skill == "roll":
            ball.velocity = direction * max(charge_speed * 0.92, 560)
            ball.charge_frames = 34
            effects.append(DuelEffect("charge", ball.position.copy(), 28, duel_attack_color("sushi"), ""))
            audio_events.append(AudioEvent(frame_index, "roll_start"))
        elif skill == "heal":
            healed = min(20, ball.max_hp - ball.hp)
            if healed > 0:
                ball.hp += healed
                effects.append(DuelEffect("heal", ball.position.copy(), 42, (144, 238, 204), f"+{healed} FRESH HEAL"))
                audio_events.append(AudioEvent(frame_index, "heal"))
            else:
                ball.velocity.rotate_ip(rng.uniform(-28, 28))
        else:
            speed = 700 if skill == "wasabi" else 620
            start = ball.position + direction * (ball.radius + 20)
            sushi_projectiles.append(
                SushiProjectile(kind=skill, position=start, velocity=direction * speed, radius=18 if skill == "wasabi" else 20, owner_skin="sushi")
            )
            effects.append(
                DuelEffect(
                    "wasabi" if skill == "wasabi" else "soy",
                    start.copy(),
                    30,
                    duel_attack_color("sushi") if skill == "wasabi" else (156, 96, 54),
                    "WASABI SHOT" if skill == "wasabi" else "SOY SPLASH",
                )
            )
            audio_events.append(AudioEvent(frame_index, "sushi_shot" if skill == "wasabi" else "soy_splash"))
        ball.skill_cooldown = 78
    elif ball.skin == "ice_cream" and ball.skill_cooldown == 0:
        direction = target.position - ball.position
        if direction.length_squared() == 0:
            direction = pygame.Vector2(1, 0)
        direction = direction.normalize()
    ...
    if ball.velocity.length_squared() > 0 and to_center.length_squared() > 0:
        ...
        edge_margin = min(
            ball.position.x - arena_rect.left,
            arena_rect.right - ball.position.x,
            ball.position.y - arena_rect.top,
            arena_rect.bottom - ball.position.y,
        )
        if (ball.skin == "sushi" and distance_ratio > 0.5) or edge_margin < ball.radius * 1.55:
            turn_strength = 0.014 if ball.skin == "sushi" else 0.007
            if edge_margin < ball.radius * 1.2:
                turn_strength *= 1.55
            current_speed = ball.velocity.length()
            ball.velocity = ball.velocity.lerp(to_center.normalize() * current_speed, min(0.12, turn_strength))
    if ball.velocity.length_squared() > 0:
        ball.visual_angle = ball.velocity.as_polar()[1] + 90
        if ball.skin == "sushi" and ball.charge_frames > 0:
            ball.visual_angle += (34 - ball.charge_frames) * 34
...
def update_cheese_patches(
    ...
    for patch in patches:
        ...
        if patch.tick_frames <= 0 and target.hp > 0:
            ...
            if final_hit:
                effects.append(DuelEffect("impact", target.position.copy(), 40, duel_attack_color("pizza"), "FINAL HIT!"))
...
def update_sushi_projectiles(
    projectiles: list[SushiProjectile],
    target: DuelBall,
    popups: list[DamagePopup],
    effects: list[DuelEffect],
    audio_events: list[AudioEvent],
    ...
    total_damage = 0
    remaining: list[SushiProjectile] = []
    for projectile in projectiles:
        projectile.position += projectile.velocity / FPS
        projectile.frames_left -= 1
        if projectile.position.distance_to(target.position) <= target.radius + projectile.radius and target.hp > 0:
            if projectile.kind == "soy":
                dealt = min(target.hp, 10)
                target.slow_frames = max(target.slow_frames, 90)
                effects.append(DuelEffect("soy", projectile.position.copy(), 36, (156, 96, 54), "SOY SPLASH"))
                audio_events.append(AudioEvent(frame_index, "soy_splash"))
                ...
                dealt = min(target.hp, 12)
                target.wasabi_frames = max(target.wasabi_frames, 120)
                target.wasabi_tick = min(target.wasabi_tick, 10) if target.wasabi_tick > 0 else 10
                effects.append(DuelEffect("wasabi", projectile.position.copy(), 36, duel_attack_color("sushi"), "WASABI SHOT"))
                audio_events.append(AudioEvent(frame_index, "sushi_hit"))
            final_hit = dealt >= target.hp
            target.hp -= dealt
            ...
            if final_hit:
                effects.append(DuelEffect("impact", target.position.copy(), 40, duel_attack_color(projectile.owner_skin), "FINAL HIT!"))
            continue
        if projectile.frames_left > 0:
            remaining.append(projectile)
    projectiles[:] = remaining
...
def apply_wasabi(ball: DuelBall, popups: list[DamagePopup], effects: list[DuelEffect], audio_events: list[AudioEvent], frame_index: int) -> int:
    if ball.wasabi_frames <= 0:
        return 0
    ball.wasabi_frames -= 1
    ball.wasabi_tick -= 1
    if ball.wasabi_tick > 0:
        return 0
    ball.wasabi_tick = 24
    damage = min(ball.hp, 4)
    ...
    if final_hit:
        effects.append(DuelEffect("impact", ball.position.copy(), 40, duel_attack_color("sushi"), "FINAL HIT!"))
    audio_events.append(AudioEvent(frame_index, "sushi_hit"))
    return damage
...
def update_ingredient_allies(
    ...
    for ally in allies:
        ...
        if ally.hit_cooldown == 0 and ally.position.distance_to(target.position) <= target.radius + ally.radius:
            ...
            if final_hit:
                effects.append(DuelEffect("impact", target.position.copy(), 40, duel_attack_color("burger"), "FINAL HIT!"))
            elif dealt >= 25:
...
def handle_duel_collision(
    ...
    if left.hit_cooldown == 0:
        ...
        if final_hit:
            effects.append(DuelEffect("impact", right.position.copy(), 42, duel_attack_color(left.skin), "FINAL HIT!"))
        elif damage >= 25:
    ...
    if right.hit_cooldown == 0:
        ...
        if final_hit:
            effects.append(DuelEffect("impact", left.position.copy(), 42, duel_attack_color(right.skin), "FINAL HIT!"))
        elif damage >= 25:
            effect_frames = 34 if damage >= 50 else 28
...
def draw_duel_result(
    surface: pygame.Surface,
    winner: DuelBall | None,
    loser: DuelBall,
    frame_index: int,
    fps: int,
    result_started_frame: int | None = None,
    tie: bool = False,
    tie_other: DuelBall | None = None,
) -> None:
    ...
    focus_food = winner or loser
    spotlight_radius = round(focus_food.radius * (2.4 + ease * 1.4))
    spotlight_color = (255, 245, 230) if tie else duel_attack_color(focus_food.skin)
    pygame.draw.circle(focus, (*spotlight_color, 54), focus_center, spotlight_radius)
    pygame.draw.circle(focus, (255, 255, 255, 210), focus_center, round(spotlight_radius * 0.72), width=6)
    surface.blit(focus, (0, 0))

    if tie and tie_other is not None:
        for offset_x, ball in [(-round(focus_food.radius * 0.92), loser), (round(focus_food.radius * 0.92), tie_other)]:
            focus_ball = DuelBall(
                name=ball.name,
                skin=ball.skin,
                color=ball.color,
                hp=ball.hp,
                max_hp=ball.max_hp,
                position=pygame.Vector2(focus_center[0] + offset_x, focus_center[1]),
                velocity=pygame.Vector2(0, 0),
                radius=round(ball.radius * (1.0 + ease * 0.35)),
        ...
        focus_ball = DuelBall(
            name=focus_food.name,
            skin=focus_food.skin,
            color=focus_food.color,
            hp=focus_food.hp,
            max_hp=focus_food.max_hp,
            position=pygame.Vector2(focus_center),
            velocity=pygame.Vector2(0, 0),
            radius=round(focus_food.radius * (1.3 + ease * 0.62)),
    ...
    if progress < 0.42:
        hit_pulse = 1 + math.sin(frame_index * 0.45) * 0.08
        draw_text_with_shadow(
            surface,
            make_font(round(110 * hit_pulse), bold=True, italic=True),
            "FINAL HIT!",
            (WIDTH // 2, round(HEIGHT * 0.235)),
            (255, 245, 230),
        )
    else:
        if tie:
            draw_text_with_shadow(surface, make_font(90, bold=True, italic=True), "UNBELIEVABLE TIE!", (WIDTH // 2, round(HEIGHT * 0.24)), (255, 245, 230))
        else:
            draw_text_with_shadow(surface, headline_font, f"{focus_food.name} WINS!", (WIDTH // 2, round(HEIGHT * 0.24)), duel_attack_color(focus_food.skin))
    if tie and tie_other is not None:
        draw_text_with_shadow(surface, result_font, "NO WINNER", (WIDTH // 2, round(HEIGHT * 0.68)), (255, 245, 230))
        draw_text_with_shadow(surface, make_font(46, bold=True), "DRAW", (WIDTH // 2, round(HEIGHT * 0.74)), (220, 232, 255))
        draw_text_with_shadow(surface, make_font(38, bold=True), f"{loser.name} HP 0   {tie_other.name} HP 0", (WIDTH // 2, round(HEIGHT * 0.80)), (255, 228, 190))
    else:
        draw_text_with_shadow(surface, result_font, "THIS FOOD WINS", (WIDTH // 2, round(HEIGHT * 0.68)), (255, 245, 230))
        draw_text_with_shadow(surface, result_font, f"Damage {focus_food.total_damage_dealt}  Hits {focus_food.hits}", (WIDTH // 2, round(HEIGHT * 0.74)), (220, 232, 255))
        draw_text_with_shadow(
            surface,
...
def run_food_duel(
    ...
    while True:
        ...
        if winner is None and not tie:
            ...
            if winner is None and not tie and frame_index > intro_frames:
                if hit_stop_frames <= 0:
                    ...
                    skill_damage_done = 0
                    skill_attacker: DuelBall | None = None
                    if left.skin in {"sushi", "ice_cream"}:
                        skill_damage_done += update_sushi_projectiles(sushi_projectiles, right, popups, duel_effects, audio_events, frame_index)
                        if left.skin == "sushi":
                            skill_damage_done += apply_wasabi(right, popups, duel_effects, audio_events, frame_index)
                        skill_attacker = left
                    elif right.skin in {"sushi", "ice_cream"}:
                        skill_damage_done += update_sushi_projectiles(sushi_projectiles, left, popups, duel_effects, audio_events, frame_index)
                        if right.skin == "sushi":
                            skill_damage_done += apply_wasabi(left, popups, duel_effects, audio_events, frame_index)
                        skill_attacker = right
                    if skill_damage_done > 0 and skill_attacker is not None:
                        skill_attacker.total_damage_dealt += skill_damage_done
                    ingredient_target = right if left.skin == "burger" else left
                    ingredient_owner = left if left.skin == "burger" else right if right.skin == "burger" else None
                    ...
                    if burger_charge_hit:
                        ...
                        shake_frames = max(shake_frames, 12)
                        shake_strength = max(shake_strength, 9)
                    elif hit_landed:
            ...
            if winner is None and not tie:
                ...
                draw_sushi_projectiles(render_surface, sushi_projectiles)
        ...
        if (winner is not None or tie) and result_started_frame is not None:
    ...
    result = {
        "status": "CLEAR",
        "theme": config.theme,
        "winner": None if tie else winner.name if winner is not None else None,
        "loser": None if tie else loser.name if loser is not None else None,
        "tie": tie,
        "left": {
            "name": left.name,
            "food": left.skin,
