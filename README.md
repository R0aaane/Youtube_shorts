...
SUSHI is available as a technical duel character. Sushi has a larger photoreal salmon nigiri sprite and cycles through visually distinct `WASABI SHOT`, `SOY SPLASH`, `ROLL DASH`, and `FRESH HEAL` effects. Wasabi uses green blobs, trails, splash, and DOT particles; Soy uses translucent brown droplets and slow rings; Roll Dash uses fast spin, pale speed lines, and a larger impact ring; Fresh Heal uses aqua rings, bubbles, and a visible heal callout.
ICE CREAM is available as a cold technical duel character with a generated realistic cup sprite that better fits the circular collision body. The default food duel config uses `BURGER vs ICE CREAM`; Ice Cream cycles through `FREEZE SHOT`, `COLD DASH`, and `SWEET HEAL` effects. Freeze attacks use icy blue shards/rings and apply slow, while Cold Dash adds a larger freeze impact. Each `FREEZE SHOT` consumes one visible scoop stage, and Ice Cream reloads for 3 seconds once the scoops are spent.
The Pizza vs Ice Cream duel uses larger lead sprites, stronger hit flash/jitter, longer cheese trails, icy freeze shards, clearer `FROZEN` status rings, red endgame edge flashes, and a one-time `FINAL PHASE` callout so Shorts viewers can read the battle state quickly.
The default Pizza vs Ice Cream pacing is tuned to finish near the 20-25 second Shorts range by using slightly higher base damage and cheese tick damage.
Burger ingredient allies now target Burger's opponent instead of whichever character is on the left side, so Burger cannot be damaged by its own lettuce, tomato, cheese, or meat allies.
If both duel characters reach 0 HP at the same time, the result screen shows `UNBELIEVABLE TIE!`, `NO WINNER`, and `DRAW` instead of assigning a winner.
