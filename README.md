...
Food duel uses realistic generated food sprites from `assets/food_sprites/`, including Burger state variants, with subtle glow and shadow so the characters stand out from the simplified arena. Collision remains circle-based internally, but the visible objects are the food sprites. Heavy hits trigger hit-stop, screen shake, and larger shockwave effects.
Damage text scales by damage amount: small ticks stay short and compact, medium hits use the standard popup, strong hits add larger text and shockwaves, and 50+ damage shows an extra `BIG HIT!` callout. Sticky cheese is drawn on top of the target food sprite as melted cheese so the ongoing Pizza effect is visible.
